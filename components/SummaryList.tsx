...
const SummaryList = ({
  ...
  const handleCopyMaterialsList = useCallback(() => {
    ...
    const text = materials
      ...
      .map(
        m =>
          `${m.item.name}: ${Math.ceil(m.quantity).toLocaleString()}${isXpMode ? " XP" : ""}`
      )
      .join("\n");
  ...
  const handleCopyShoppingList = useCallback(() => {
    const shoppingList = materials
      ...
      .sort(
        (a, b) =>
          b.item.tier - a.item.tier || a.item.name.localeCompare(b.item.name)
      )
  ...
  const getItemPrice = (itemName: string, quantity: number) => {
    if (!showPrices || !priceConfig?.enabled || !itemName || !_priceData)
      return null;
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-3">
                <div
                  ...
                  onClick={() => {
                    if (item.id === "GEMSTONE_DUST" && onIngredientChange) {
                      ...
                      const current =
                        selectedIngredients?.[item.id] || "PRISTINE_AMBER";
                ...
                >
                  <img
                    src={getIconUrl(item.id, item.tier)}
                    alt={item.name}
                ...
                <div>
                  ...
                  {showInventory &&
                    ...
                    inInventory !== undefined && (
                      <p className="text-xs text-gray-400">
                        Need: {Math.ceil(originalQuantity).toLocaleString()} |
                        Have: {inInventory.toLocaleString()}
                      </p>
                    )}
                </div>
              </div>
                ...
                {showInventory && (
                  <div className="text-right">
                    {isEditing ? (
                      ...
                      <button
                        onClick={() =>
                          handleInventoryEdit(item.id, inventoryAmount)
                        }
