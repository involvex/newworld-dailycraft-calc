...
const useInventoryOCR = ({
  ...
  const processImage = useCallback(async (imageSrc: string) => {
    try {
      ...
      if (debugOCRPreview) { // Use debugOCRPreview prop
      ...
      console.log('analyzing image using gemini api with API_KEY :' + geminiApiKey);
      const analyzedItems: AnalyzedItem[] = await analyzeInventoryImage(base64Image, geminiApiKey); // Use geminiApiKey prop
  ...
  }, [geminiApiKey, debugOCRPreview, setIsProcessingOCR, setOCREditText, setShowOCREdit]);
  ...
  }, [processImage, setOCREditText, setShowOCREdit]);

    const parseManualInventory = (text: string): Record<string, number> => {
      const foundItems: Record<string, number> = {};
      const lines = text.split(/[\n\r]+/).filter(line => line.trim().length > 0);      for (const line of lines) {
        const match = line.match(/^(.+?)\s*[:xX]?\s*(\d{1,6}(?:[.,]\d{1,3})?)\s*$/);
        if (match) {
