...
export default function useTreeCollapse({
  collapsedNodes,
  setCollapsedNodes
}: UseTreeCollapseProps) {
  ...
  const handleCollapseAll = useCallback(
    ...
    [setCollapsedNodes]
  ); // Added setCollapsedNodes to dependencies
  ...
  const restoreCollapsedNodes = useCallback(
    (nodes: string[] | Set<string>) => {
  ...
  const handleCollapseToNode = useCallback(
    (_rootNode: any, targetNodeId: string) => {
      setCollapsedNodes((prev: Set<string>) => {
        const newSet = new Set(prev);
  ...
  const handleExpandToNode = useCallback(
    (rootNode: any, targetNodeId: string) => {
