  ...
  saveConfig(config) {
    try {
      ...
      if (configToSave.GEMINI_API_KEY) {
        configToSave.encryptedGeminiApiKey = this.encrypt(
          configToSave.GEMINI_API_KEY
        );
  ...
  mergeWithDefaults(config) {
    ...
    if (config.settings) {
      ...
      if (config.settings.bonuses) {
        merged.settings.bonuses = {
          ...merged.settings.bonuses,
          ...config.settings.bonuses
        };
      }
    }
    ...
    merged.quickNotes = config.quickNotes || [];
