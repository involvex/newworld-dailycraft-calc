...
async function fetchItemDefinitions(): Promise<ItemDefinitionMap> {
  ...
  const itemDefs = await fetchJsonWithProxy<ItemDefinitionMap>(ITEM_DEFS_API_URL);
  
  if (itemDefs === null) {
    console.error("Could not fetch item definitions. This is a critical failure. Market data may be incorrect.");
  ...
  itemDefinitionsCache = Object.entries(itemDefs).reduce<ItemDefinitionMap>((acc, [key, value]) => {
 ...
 */
export async function fetchPricesByItemName(serverName: string): Promise<Map<string, number>> {
